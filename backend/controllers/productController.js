...
const uploadToCloudinary = (file) => {
  return new Promise((resolve, reject) => {
    ...
    const uploadStream = cloudinary.uploader.upload_stream(
      { folder: 'aurumvault/products', resource_type: isVideo ? 'video' : 'image', timeout: 120000 },
      (error, result) => {
...
exports.createProduct = async (req, res) => {
  try {
    ...
    const newProduct = new Product({
      ...
      onDiscount: onDiscount === 'true' || onDiscount === true,
      discountPercentage,
      salePrice,
      media: mediaArray,
    });
...
exports.updateProduct = async (req, res) => {
  try {
    ...
    // Handle FormData checkbox omissions (unchecked boxes aren't sent at all)
    updateData.isNewArrival = updateData.isNewArrival === 'true';
    updateData.onDiscount = updateData.onDiscount === 'true';
    ...
    if (req.files && req.files.length > 0) {
      ...
      newMediaArray = await Promise.all(uploadPromises);
    }
    ...
    if (newMediaArray.length > 0) {
       updateData.media = [...existingProduct.media, ...newMediaArray];
    }
...
exports.deleteProductMedia = async (req, res) => {
  try {
    ...
    if (!public_id) {
      return res.status(400).json({ message: 'public_id query parameter is required' });
    }
    ...
    if (!mediaToRemove) return res.status(404).json({ message: 'Media not found in product' });
...
exports.getAllProducts = async (req, res) => {
  try {
    const { category, gender, quality } = req.query;
    let query = {};
    
    if (category) query.category = new RegExp(`^${category}$`, 'i');
    if (gender) {
      if (gender.toLowerCase() === 'male' || gender.toLowerCase() === 'female') {
        query.gender = { $in: [new RegExp(`^${gender}$`, 'i'), new RegExp('^unisex$', 'i')] };
      } else {
        query.gender = new RegExp(`^${gender}$`, 'i');
      }
    }
    if (quality) query.quality = new RegExp(`^${quality}$`, 'i');
